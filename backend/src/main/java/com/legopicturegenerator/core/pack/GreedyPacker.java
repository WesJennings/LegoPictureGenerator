...
public class GreedyPacker {
  ...
  private List<PlacedPart> packColMajor(
      ColorMatcher.LegoElement[][] studs, int w, int h) {
    boolean[][] covered = new boolean[h][w];
    List<PlacedPart> placed = new ArrayList<>();
    for (int x = 0; x < w; x++) {
      for (int y = 0; y < h; y++) {
  ...
  private List<PlacedPart> repairLeftovers(
      ColorMatcher.LegoElement[][] studs, int w, int h, List<PlacedPart> initial) {
    boolean[][] covered = new boolean[h][w];
  ...
  static boolean fits(
    ...
    for (int dy = 0; dy < ph; dy++) {
      for (int dx = 0; dx < pw; dx++) {
        ...
        ColorMatcher.LegoElement el = studs[y + dy][x + dx];
