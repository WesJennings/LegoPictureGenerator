...
**Docs split:** algorithm explanations live **here**. High-level types, wiring, BOM, and research papers live in [`README.md`](README.md).
...
```text
  ...
  GreedyPack ExactIlp   RlePack  ComponentG  DlxPack  AnnealPack
      ...
      └──────────┴──────────┴────┬─────┴──────────┴──────────┘
              ...
              (artifacts/bom_*.txt, output_lego_*.png)
```
...
Each mode returns a `PackResult` → BOM text + packed PNG. Wiring, status strings, and file roles live in [`README.md`](README.md) (high-level); this walkthrough stops at *how* each packer builds the placement list.
